...
// Masonry 레이아웃을 위한 라이브러리만 import
...
// Masonry 레이아웃에 필요한 반응형 break point 설정
const breakpointColumnsObj = {
  default: 3, // 기본 3열
  1100: 2, // 1100px 미만 시 2열
  700: 1, // 700px 미만 시 1열
};
...
        className="my-masonry-grid flex w-auto mb-12" // Flexbox와 폭 설정
        columnClassName="my-masonry-grid_column bg-clip-padding pl-4" // 컬럼에 패딩을 줘서 갭 생성
      >
        {images.map(img => (
          <div key={img.id} className="mb-4">
            {/* 이미지 영역: hover 시 확대 효과를 추가하여 '재미' 부여 */}
...
                className="w-full h-auto block transform group-hover:scale-[1.02] transition-transform duration-500 ease-in-out cursor-pointer" // 확대 슬라이드 효과
              />
            </div>
            {img.description && (
              <p className="text-sm text-gray-500 mt-2 p-2">{img.description}</p>
            )}
          </div>
        ))}
      </Masonry>
