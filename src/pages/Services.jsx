...
export default function Services() {
  const fields = [
    ...
    {
      icon: <GalleryHorizontalEnd className="w-8 h-8" />,
      title: "전시·박람회",
      ...
      <section className="grid md:grid-cols-3 gap-8 mb-24">
        {fields.map((f, i) => (
          <motion.div
            ...
            viewport={{ once: true }}
            className="bg-gray border border-gray-200 p-10 rounded-xl shadow-sm hover:shadow-md transition text-center"
          >
        ...
        <div className="grid md:grid-cols-4 gap-10">
          {steps.map((step, index) => (
            <motion.div
              ...
              viewport={{ once: true }}
              className="text-center border border-gray-200 bg-gray p-8 rounded-xl shadow-sm hover:shadow-md transition"
            >
              <div className="mx-auto text-primary mb-4">{step.icon}</div>
              <h3 className="font-semibold text-lg mb-2 text-gray-200">{step.title}</h3>
